...
func (k msgServer) SubmitScavenge(goCtx context.Context, msg *types.MsgSubmitScavenge) (*types.MsgSubmitScavengeResponse, error) {
        // get context that contains information about the environment, such as a block height
	...
	var scavenge = types.Scavenge{
        ...
        // convert the message creator address from a string to sdk.AccAddress
        scavenger, err := sdk.AccAddressFromBech32(msg.Creator)
        if err != nil {
          panic(err)
        }

        // convert token from string into sdk.Coins
